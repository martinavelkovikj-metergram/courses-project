...
export class Participants{
    async createParticipant(participantParams:ParticipantParams, companyId:number){
        try{
            return await new ParticipantService().createParticipant(participantParams, companyId);
        }catch(err){
            console.error(err)
        }
    }
    ...
    async getAllParticipantsFromCompany(companyId: number){
        try{
            return await new ParticipantService().getAllParticipantsFromCompany(companyId);
        }catch(err){
            console.error(err)
        }
    }
